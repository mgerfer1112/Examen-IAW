...
mysql -u root <<< "DROP USER IF EXISTS $MOODLE_DB_USER@$FRONTEND_PRIVATE_IP"
mysql -u root <<< "CREATE USER $MOODLE_DB_USER@$FRONTEND_PRIVATE_IP IDENTIFIED BY '$MOODLE_DB_PASSWORD'"
